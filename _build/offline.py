...
<p>We ran three of them against a real product description, on the same
machine, and counted. This is the whole of what we know:</p>
<ul>
<li><strong>Qwen2.5 1.5B — 0 of 7.</strong> Never produced a valid campaign.
It leaves 13 required fields out at once, which is not a near miss.</li>
<li><strong>Llama 3.2 3B — 0 of 2.</strong> Same shape, 11 fields short.</li>
<li><strong>Qwen2.5 7B — 2 of 2.</strong> Both times it came back one field
short and the app patched that field rather than throwing the answer away.
Before that repair existed it was 0 of 2.</li>
</ul>
<p>Two attempts each is a small sample and we are giving you the counts rather
than a percentage. The 4B, 8B and 30B have not been run against the campaign
schema at all, and we would rather say so than imply they sit on a line
between the ones we did test.</p>
<p>The practical reading: <strong>on this evidence the floor is around 7B</strong>,
which needs about 12GB of memory. The smaller models will download, load and
write fluent English, and then fail to produce a campaign. They stay in the
list with that result attached, because someone who reads about them elsewhere
should find this out here rather than after an hour.</p>
<p>If you have an API key or Outlier running, use those.</p>
