...
particular job.</p>
<p>The one measurement we do have is unflattering: running the smallest model,
{_esc(smallest['name'])}, on a real product page produced a complete campaign
document and <strong>one ad variant where three were asked for</strong>. A
three-cell test is the entire point of the exercise, so on that machine the
built-in model gets you a usable draft and not a usable test. The larger
models in the table have more room; how much more, we have not measured.</p>
<p>If you have an API key or Outlier running, use those. This is the floor,
and the floor exists so that having neither still gets you something.</p>
