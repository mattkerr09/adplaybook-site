...
call sites; it no longer holds a blue and the name is legacy. */
  ...
  --grad-mid:#8FD9F2;
  --glow-1:rgba(143,217,242,.30);--glow-2:rgba(95,182,228,.22);
  --glow-3:rgba(223,246,255,.16);
...
:root[data-theme="light"]{
  ...
  --grad-mid:#155E75;
  --glow-1:rgba(21,94,117,.12);--glow-2:rgba(14,116,144,.10);
  --glow-3:rgba(29,110,142,.08);
...
/* Ambient light behind the hero. outlier.host layers three radial blobs plus a
   1px dot grid at 5% white; this is the same construction at lower opacity,
   because our ground is neutral and a strong wash on it reads as a stain. */
.hero{position:relative}
.hero::before{content:"";position:absolute;inset:-10% -20% 20%;z-index:-1;
  pointer-events:none;
  background:
    radial-gradient(38% 44% at 18% 12%,var(--glow-1),transparent 68%),
    radial-gradient(32% 38% at 78% 6%,var(--glow-2),transparent 66%),
    radial-gradient(30% 36% at 52% 40%,var(--glow-3),transparent 70%)}
...
/* Pay-in-4. The copy lives in _build/bnpl.py and is NOT published — the Buy
   ...
   the build if the copy reaches a page before the button moves to Dodo.
