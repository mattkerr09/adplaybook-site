...
:root{
  --black:#080b09;--panel:#0c110e;--panel-2:#101613;--card:#141b17;
  ...
  /* Four of the five sites in this portfolio were near-black with a blue
     ...
     trace back to the advertiser's own page. So the accent becomes the green
     it already used for VERIFIED, and display type becomes the mono it already
     used for kickers. --blue is kept as an alias rather than renamed at 40+
     call sites; it no longer holds a blue and the name is legacy. */
  --accent:#4ade80;--blue:var(--accent);--ice:#a8c8ff;--violet:#9d8cff;--green:#4ade80;--amber:#d9a72b;
  ...
  --nav-bg:rgba(6,8,12,.72);--nav-hair:rgba(28,32,41,.9);
  --on-accent:#04140b;                      /* text ON the green button */
  --accent-lift:#6ee7a0;--accent-deep:#2fa864;
  --accent-glow:rgba(74,222,128,.85);--accent-glow-2:rgba(74,222,128,1);
  --accent-wash:rgba(74,222,128,.16);--accent-wash-2:rgba(74,222,128,.13);
  --accent-edge:rgba(74,222,128,.5);--amber-edge:rgba(217,167,43,.5);
...
:root[data-theme="light"]{
  --black:#f7f9f7;--panel:#ffffff;--panel-2:#eef2ef;--card:#ffffff;
  --hair:#e0e6e2;--hair-lit:#c7d0ca;
  --white:#0c120e;--grey:#4d5a52;--grey-dim:#67736c;
  --accent:#15803d;--ice:#1d4ed8;--violet:#6d28d9;--green:#15803d;--amber:#8a5a12;
  --nav-bg:rgba(247,249,247,.82);--nav-hair:rgba(200,208,203,.9);
  --on-accent:#ffffff;
  --accent-lift:#22a555;--accent-deep:#127034;
  --accent-glow:rgba(21,128,61,.34);--accent-glow-2:rgba(21,128,61,.42);
  --accent-wash:rgba(21,128,61,.10);--accent-wash-2:rgba(21,128,61,.08);
  --accent-edge:rgba(21,128,61,.45);--amber-edge:rgba(138,90,18,.45);
  --row-hover:rgba(12,18,14,.035);
...
h2{font-family:var(--mono);font-size:clamp(1.45rem,2.7vw,2rem);line-height:1.18;letter-spacing:-.014em;
...
@media(max-width:640px){
  ...
  .nav-links a, footer a{display:inline-flex;align-items:center;min-height:24px}
}
"""

LOGO_SVG = (
    '<svg class="nav-logo" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">'
    '<rect width="100" height="100" rx="22" fill="#101613"/>'
    '<rect x="14" y="14" width="14" height="72" rx="4" fill="#4ade80"/>'
    '<rect x="33" y="14" width="53" height="72" rx="6" fill="#4ade80"/>'
    '<circle cx="46" cy="70" r="9.5" fill="#101613"/><circle cx="46" cy="70" r="4.3" fill="#4ade80"/>'
    '<circle cx="61" cy="53" r="5" fill="#101613"/><circle cx="75" cy="36" r="5" fill="#101613"/>'
    "</svg>"
)
...
download — there is no price, no checkout and no licence key.</p>
</div></footer>
